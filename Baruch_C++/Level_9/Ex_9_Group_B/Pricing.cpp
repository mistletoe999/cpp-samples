// Pricing.cpp
// Implements the functions defined in Pricing.hpp
...
namespace OPTIONPRICING
{
	namespace PRICING
	{
		...
		// Perpetual American Option Price Formula
		// With all parameters
		...
		{
			double yValue1 = y1(volatility, interestRate, costOfCarry);
			double yValue2 = y2(volatility, interestRate, costOfCarry);

			switch (optionType)
			{
			case AMERICANPERPETUALCALL:
				return ((strikePrice/(yValue1 - 1))*std::pow(((yValue1 - 1)*stockPrice/(yValue1*strikePrice)), yValue1));
				break;
			case AMERICANPERPETUALPUT:
				return ((strikePrice / (1 - yValue2))*std::pow(((yValue2 - 1)*stockPrice / (yValue2*strikePrice)), yValue2));
				break;
			default:
				throw EXCEPTIONS::InvalidOptionException(-1);
				break;
		...
		// Overloaded function with OptionData reference as parameter

		double PerpetualAmericanOptionPrice(const OptionData& data,	const double costOfCarry, 
			const OptionType optionType)
		{
			double yValue1 = y1(data.sig, data.r, costOfCarry);
			double yValue2 = y2(data.sig, data.r, costOfCarry);

			std::cout << yValue1 << std::endl;

			switch (optionType)
			{
			case AMERICANPERPETUALCALL:
				return ((data.K / (yValue1 - 1))*std::pow(((yValue1 - 1)*data.S / (yValue1*data.K)), yValue1));
				break;
			case AMERICANPERPETUALPUT:
				return ((data.K / (1 - yValue2))*std::pow(((yValue2 - 1)*data.S / (yValue2*data.K)), yValue2));
				break;
			default:
				throw EXCEPTIONS::InvalidOptionException(-1);
				break;
