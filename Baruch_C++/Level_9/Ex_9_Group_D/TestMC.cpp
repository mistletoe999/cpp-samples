...
// EDIT:
// Added BlackScholes.hpp to get the BlackScholes price for
// the required batches
// OptionException class is the exception class for the option pricing
// InvalidOptionException is a derived class from OptionException
// StandardError.hpp contains the Error function to calculate the
// MonteCarlo standard deviation and standard error
...
int main()
{
	...
	// EDIT
	// Vector to add prices to feed into Error function
	...
	// Displays the Actual Price for comparison
	std::cout << "Actual price of the option: " << BlackScholesPrice(S_0,
		myOption.K, myOption.T, myOption.sig, myOption.r, myOption.r, EUROPEANCALL) << std::endl;

	// Display the Monte Carlo Price
	std::cout << "Price, after discounting: " << price << ", " << std::endl;
	std::cout << "Number of times origin is hit: " << coun << endl;

	// Display the Standard Deviation and Standard Error on the console
	boost::tuple<double, double> standardErrors;

	standardErrors = Error(prices, myOption.r, myOption.T);

	cout << "SD: " << get<0>(standardErrors) << "; SE:" << get<1>(standardErrors) << endl;
