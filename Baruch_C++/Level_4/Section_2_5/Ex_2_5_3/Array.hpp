...
public:
	...
	void SetElement(unsigned int index, const Point& p); // Sets m_data[index] to p
	Point& GetElement(unsigned int index) const; // Gets the refernce to m_data[index]

	// Operators
	Array& operator=(const Array& source); // Assignment Operator	
	Point& operator [] (int index);	// Square bracket index operator  
	const Point& operator [] (int index) const; // Square bracket with index operator (const reference and const output): 
